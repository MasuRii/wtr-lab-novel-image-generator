...
export const App = {
    ...
    async init() {
        try {
            ...
            logger.logInfo('APP', 'WTR Lab Novel Image Generator initialized successfully');
        } catch (error) {
            logger.logError('APP', 'Failed to initialize application', { error: error.message });
            throw error;
        }
    },
    ...
    async cleanupHistory() {
        try {
            ...
            if (removedCount > 0) {
                logger.logInfo('APP', `Cleaned ${removedCount} old history entries on startup`);
            }
        } catch (error) {
            logger.logError('APP', 'Failed to clean old history entries', { error: error.message });
        }
    },
