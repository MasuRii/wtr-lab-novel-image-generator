...
export async function enhancePromptWithGemini(originalPrompt, config) {
    ...
    for (let modelIndex = 0; modelIndex < modelsList.length; modelIndex++) {
        ...
        while (attemptsForThisModel < enhancementMaxRetriesPerModel) {
            ...
            try {
                ...
                lastError = error;
                logError('ENHANCEMENT', `Enhancement failed for model ${model} (attempt ${attemptsForThisModel}/${enhancementMaxRetriesPerModel})`, {
                    model,
        ...
        logWarn('ENHANCEMENT', `Exhausted retries for model ${model}, switching to next model`, {
            model,
    ...
    // All models failed
    const duration = Date.now() - startTime;
    logError('ENHANCEMENT', 'All models and retries exhausted for prompt enhancement', {
