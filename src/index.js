...
(function () {
    ...
    function retryGeneration(prompt, provider, providerProfileUrl = null) {
        generationQueue.unshift({ prompt, provider, providerProfileUrl });
        isGenerating = false;
        errorModal.hide();
    ...
    async function processQueue() {
        ...
        switch (request.provider) {
            case 'Google': apiGoogle.generate(request.prompt, callbacks); break;
            case 'AIHorde': apiAIHorde.generate(request.prompt, callbacks); break;
            case 'Pollinations': apiPollinations.generate(request.prompt, callbacks); break;
            case 'OpenAICompat': apiOpenAI.generate(request.prompt, request.providerProfileUrl, callbacks); break;
            default: handleGenerationFailure(`Unknown provider: ${request.provider}`, request.prompt, 'System');
        }
    }
    ...
    async function onGenerateClick() {
        ...
        logger.logInfo('GENERATION', 'Starting image generation process', { selectionLength: currentSelection.length });
        ...
        retryGeneration(finalPrompt, config.selectedProvider, config.openAICompatActiveProfileUrl);
    }
    ...
    async function init() {
        ...
        errorModal.create();
        configPanel.create();
        errorModal.init({ onRetry: retryGeneration });
