...
async function getCachedModels() {
    try {
        ...
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to parse cached models data, resetting cache', { error: error.message });
...
async function getCachedModelsForProvider(provider) {
    try {
        ...
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', `Failed to get cached models for ${provider}`, { error: error.message });
        return null;
    }
}
...
async function setCachedModels(provider, models) {
    try {
        ...
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to cache models', { provider, error: error.message });
        ...
        try {
            ...
            (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to cache models even after reset', { provider, error: retryError.message });
        }
    }
}
...
async function clearCachedModels(provider = null) {
    try {
        if (provider) {
            ...
            (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .logInfo */ .fH)('CACHE', 'Cleared all cached models and reset OpenAI Compatible model selections.');
            alert('All cached models have been cleared. They will be re-fetched when you next open the settings.');
        }
    } catch (error) {
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to clear cached models', { provider, error: error.message });
        ...
        try {
            ...
            (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to force reset cache', { error: fallbackError.message });
        }
    }
}
...
async function getCachedOpenAICompatModels(profileUrl) {
    try {
        ...
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to parse OpenAI compatible models cache', { profileUrl, error: error.message });
        return [];
    }
}
...
async function setCachedOpenAICompatModels(profileUrl, models) {
    try {
        ...
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('info', 'CACHE', `Cached models for OpenAI compatible provider: ${profileUrl}`);
    } catch (error) {
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to cache OpenAI compatible models', { profileUrl, error: error.message });
    }
}
...
async function clearCachedOpenAICompatModels() {
    try {
        ...
        try {
            ...
            (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to get OpenAI profiles for cache clearing', { error: error.message });
            return;
        }
        ...
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .logInfo */ .fH)('CACHE', 'Cleared cached models for all OpenAI compatible providers.');
    } catch (error) {
        (0,_logger_js__WEBPACK_IMPORTED_MODULE_0__/* .log */ .Rm)('error', 'CACHE', 'Failed to clear OpenAI compatible model caches', { error: error.message });
    }
}
...
async function fetchPollinationsModels(selectedModel) {
    ...
    try {
        ...
        logger/* logError */.vV('CACHE', 'Failed to get cached Pollinations models', { error: error.message });
    }
    ...
    GM_xmlhttpRequest({
        ...
        onload: async (response) => {
            try {
                ...
                logger/* logInfo */.fH('NETWORK', 'Fetched and cached Pollinations models', { count: models.length });
                populatePollinationsSelect(select, models, selectedModel);
            } catch (e) {
                logger/* logError */.vV('NETWORK', 'Failed to parse Pollinations models', { error: e.message });
                select.innerHTML = '<option>flux</option>';
                select.value = 'flux';
            }
        },
        onerror: (error) => {
            logger/* logError */.vV('NETWORK', 'Failed to fetch Pollinations models', { error: error });
            select.innerHTML = '<option>flux</option>';
            select.value = 'flux';
        }
    });
...
async function fetchAIHordeModels(selectedModel) {
    ...
    try {
        ...
        logger/* logError */.vV('CACHE', 'Failed to get cached AI Horde models', { error: error.message });
    }
    ...
    GM_xmlhttpRequest({
        ...
        onload: async (response) => {
            try {
                ...
                logger/* logInfo */.fH('NETWORK', 'Fetched and cached AI Horde models', { count: apiModels.length });
                populateAIHordeSelect(select, apiModels, selectedModel);
            } catch (e) {
                logger/* logError */.vV('NETWORK', 'Failed to parse AI Horde models', { error: e.message });
                select.innerHTML = '<option>Stable Diffusion</option>';
                select.value = 'Stable Diffusion';
            }
        },
        onerror: (error) => {
            logger/* logError */.vV('NETWORK', 'Failed to fetch AI Horde models', { error: error });
            select.innerHTML = '<option>Stable Diffusion</option>';
            select.value = 'Stable Diffusion';
        }
    });
    ...
    GM_xmlhttpRequest({
        ...
        onload: async (response) => {
            try {
                ...
                logger/* logError */.vV('NETWORK', 'Failed to parse OpenAI compatible models', { error: error.message });
        ...
        onerror: (error) => {
            logger/* logError */.vV('NETWORK', 'Failed to fetch OpenAI compatible models', { error });
            select.innerHTML = '<option>Failed to fetch models</option>';
...
// Module
___CSS_LOADER_EXPORT___.push([module.id, `/* === CSS Custom Properties (Design Tokens) === */
:root {
...
function show(imageUrls, prompt, provider, model = 'Unknown') {
	...
	imageUrls.forEach((url, index) => {
		...
		if (!isBase64Image(url) && isValidUrl(url)) {
			urlLinkBtn.innerHTML = '<span class="material-symbols-outlined">link</span>';
...
.nig-password-toggle {
    ...
    cursor: pointer;
    color: var(--nig-color-text-muted);
    transition: color var(--nig-transition-fast), transform var(--nig-transition-fast);
}
...
function shouldUseProviderEnhancement(provider, config) {
    (0,logger/* logDebug */.MD)('ENHANCEMENT', 'Checking provider priority for enhancement', { provider, config });
...
async function enhancePromptWithGemini(originalPrompt, config) {
    ...
    const modelsList = [rawModel, ...enhancementModelsFallback.filter(m => m !== rawModel)];
    ...
    for (let modelIndex = 0; modelIndex < modelsList.length; modelIndex++) {
        ...
        const model = modelWithPrefix.startsWith('models/') ? modelWithPrefix.substring(7) : modelWithPrefix;
        ...
        while (attemptsForThisModel < enhancementMaxRetriesPerModel) {
            ...
            try {
                ...
                lastError = error;
                (0,logger/* logError */.vV)('ENHANCEMENT', `Enhancement failed for model ${model} (attempt ${attemptsForThisModel}/${enhancementMaxRetriesPerModel})`, {
                    model,
        ...
        (0,logger/* logWarn */.JE)('ENHANCEMENT', `Exhausted retries for model ${model}, switching to next model`, {
            model,
 ...
 */
async function attemptEnhancementWithModel(originalPrompt, model, template, apiKey, isPrimaryModel, attemptNumber, maxRetries) {
...
// EXTERNAL MODULE: ./src/utils/cache.js
var src_cache = __webpack_require__(168);
        ...
        onload: async (response) => {
            if (response.status >= 200 && response.status < 300) {
                ...
                if (text.toLowerCase().includes('model not found')) {
                    ...
                    (0,src_cache/* clearCachedModels */.WN)('pollinations');
                    return;
                }
            ...
            try {
                ...
                if (data.id) {
                    ...
                    if (data.message && data.message.toLowerCase().includes('model')) {
                        ...
                        (0,src_cache/* clearCachedModels */.WN)('aiHorde');
                        return;
                    }
...
async function show(errorDetails) {
    ...
    retryBtn.onclick = () => {
        ...
        try {
            ...
            logger/* logError */.vV('ERROR_MODAL', 'Retry callback threw an error', { error: e && e.message });
        }
        hide();
    };
...
function pollinationsAuthPrompt_show(errorMessage, failedPrompt, onRetry) {
    ...
    pollinationsAuthPrompt_promptElement.querySelector('.nig-close-btn').addEventListener('click', close);
    pollinationsAuthPrompt_promptElement.querySelector('#nig-prompt-save-token-btn').addEventListener('click', async () => {
        const token = pollinationsAuthPrompt_promptElement.querySelector('#nig-prompt-pollinations-token').value.trim();
        if (token) {
    ...
    if (templateDeletePresetBtn) {
        templateDeletePresetBtn.addEventListener('click', async () => {
            try {
                ...
                const fallbackKey = defaults/* DEFAULTS */.z.enhancementPresets?.standard ? 'standard' : 'custom';
...
function normalizeImportedConfig(importedConfigRaw = {}) {
    try {
        ...
        if (!('enhancementTemplateSelected' in importedConfig)) {
            ...
            if (importedTemplate && typeof importedTemplate === 'string' && defaults/* DEFAULTS */.z.enhancementPresets) {
            ...
            if (!sel || (sel !== 'custom' && !presets[sel])) {
                normalized.enhancementTemplateSelected = defaults/* DEFAULTS */.z.enhancementTemplateSelected || 'standard';
            }
        }
        ...
        if (isNonEmptyString(importedConfig.openAICompatActiveProfileUrl)) {
            ...
            normalized.openAICompatActiveProfileUrl = defaults/* DEFAULTS */.z.openAICompatActiveProfileUrl;
        }
        ...
        if (typeof importedConfig.openAICompatModelManualInput === 'boolean') {
            ...
            normalized.openAICompatModelManualInput = defaults/* DEFAULTS */.z.openAICompatModelManualInput;
        }
        ...
        logger/* logError */.vV('CONFIG_IMPORT', 'Failed to normalize imported config', { error: error.message });
...
function initializePasswordVisibilityToggles(panelElement) {
    try {
        ...
        toggles.forEach((toggleBtn) => {
            ...
            toggleBtn.addEventListener('click', (event) => {
                try {
                    ...
                    try {
                        logger/* logError */.vV('UI', 'Failed to toggle API key visibility', { error: err.message });
                    } catch (_) {
        ...
        try {
            logger/* logError */.vV('UI', 'Failed to initialize API key visibility toggles', { error: error.message });
        } catch (_) {
...
function setupOpenAIEventListeners(panelElement) {
    ...
    panelElement.querySelector('#nig-openai-compat-profile-select').addEventListener('change', models/* loadSelectedOpenAIProfile */.tH);
    
    // OpenAI Compatible delete profile
    panelElement.querySelector('#nig-openai-compat-delete-profile').addEventListener('click', models/* deleteSelectedOpenAIProfile */.CB);
...
function setupUtilityEventListeners(panelElement) {
    ...
    // Clear cache
    panelElement.querySelector('#nig-clear-cache-btn').addEventListener('click', () => src_cache/* clearCachedModels */.WN());
}
...
function configPanel_create() {
    ...
    panelElement.querySelector('#nig-save-btn').addEventListener('click', configPanel_saveConfig);
...
async function configPanel_show() {
    if (!panelElement) configPanel_create();
...
(function () {
    ...
    function handleGenerationSuccess(displayUrls, prompt, provider, model, persistentUrls = null) {
        ...
        historyUrls.forEach(url => storage/* addToHistory */.Pc({ date: new Date().toISOString(), prompt, url, provider, model }));
        isGenerating = false;
        updateSystemStatus();
        processQueue();
    }
    ...
    function handleGenerationFailure(errorMessage, prompt = 'Unknown', provider, providerProfileUrl = null, errorMetadata = null) {
        logger/* logError */.vV('GENERATION', `Generation Failed`, { prompt, provider, errorMessage, errorMetadata });
    ...
    async function processQueue() {
        ...
        const callbacks = {
            ...
            onAuthFailure: (msg, p) => {
                logger/* logInfo */.fH('AUTH', 'Authentication required', { provider: p, message: msg });
        ...
        if (config.enhancementEnabled) {
            ...
            if (shouldUseExternalEnhancement) {
                ...
                try {
                    ...
                    logger/* logError */.vV('ENHANCEMENT', 'External AI enhancement failed, falling back to original', { error: error.message });
